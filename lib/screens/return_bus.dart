import 'dart:async';
import 'package:event_entry/screens/return_bus_details.dart';
...
class ReturnBusTicket extends StatefulWidget {
  @override
  _ReturnBusTicketState createState() => _ReturnBusTicketState();
}

class _ReturnBusTicketState extends State<ReturnBusTicket> {
  ...
  Widget build(BuildContext context) {
    return loading
        ...
        : Scaffold(
            key: _scaffoldKey,
            appBar: CustomAppBar("Return Bus Ticket"),
            body: Builder(
    ...
    if (_outputController.text.isEmpty) {
      ...
      await dataSnapshot.once().then(
            (data) => {
              if (data.value != null)
                {
                  status = data.value['bus_return_ticket'],
                  if (status == 'true')
                    {
                      ...
                      Navigator.push(
                        context,
                        MaterialPageRoute(
                            builder: (context) => ReturnBusTicketDetails(
                      ...
                      Navigator.push(
                        ...
                        MaterialPageRoute(
                          builder: (context) => ErrorPage(
                              title: "User ticket has been already used"),
                        ),
                      ),
                    }
                }
